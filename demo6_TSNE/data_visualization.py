...
if __name__ == "__main__":
    ...
    reducer = umap.UMAP(random_state=42, n_components=2, n_neighbors=15, init='random')
    t0 = time()
    X_umap = reducer.fit_transform(X)
    print(X_umap.shape)
    plot_embedding_2d(X_umap[:, 0:2], "umap 2D")
    # plot_embedding_3d(X_umap, "umap 3D (time %.2fs)" % (time() - t0))
